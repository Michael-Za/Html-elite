  ...
  try {
    ...
    try {
      ...
      await sendEmail({
        ...
        text: `Hello${name ? ` ${name}` : ''},\n\nYour account has been created successfully!\n\nWorkspace: ${user.tenant.name}\nRole: ${userRole}\n\nYou can now log in and start using Klickbee CRM.\n\nIf you didn't create this account, please ignore this email.`,
        html: `<p>Hello${name ? ` ${name}` : ''},</p><p>Your account has been created successfully!</p><p><strong>Workspace:</strong> ${user.tenant.name}</p><p><strong>Role:</strong> ${userRole}</p><p>You can now <a href="${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/auth">log in</a> and start using Klickbee CRM.</p><p>If you didn't create this account, please ignore this email.</p>`,
      })
    } catch (e) {
